...
async function SnippetRunner() {
  ...
  function renderMarkdown(mdText) {
    ...
    function inlineMd(safeText) {
      ...
      safeText = safeText.replace(
        /\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g,
        (_, txt, url) => {
          return `<a class="sr-a" href="${esc(
            url
          )}" target="_blank" rel="noreferrer noopener">${esc(txt)}</a>`;
        }
      );
      return safeText;
    }
  ...
  async function fetchText(url, { allowRetryWithTokenPrompt = true } = {}) {
    ...
    if (
      !res.ok &&
      allowRetryWithTokenPrompt &&
      CONFIG.githubToken === "prompt"
    ) {
  ...
  async function fetchViaGithubApi(rawUrlThatFailed) {
    ...
    const path = rawUrlThatFailed.startsWith(marker)
      ...
      : null;
    if (!path)
      throw new Error("Could not map raw URL to repo path for API fallback.");

    const apiUrl = `https://api.github.com/repos/${CONFIG.owner}/${
      CONFIG.repo
    }/contents/${path}?ref=${encodeURIComponent(CONFIG.ref)}`;
    ...
    if (!res.ok) {
      const body = await safeRead(res);
      throw new Error(
        `GitHub API fetch failed (${res.status}) for ${apiUrl}\n${
          body ? "Details: " + body : ""
        }`
      );
    }
  ...
  const header = createEl("div", { class: "sr-header" }, [
    ...
    createEl("div", { class: "sr-actions" }, [
      createEl(
        "button",
        { class: "sr-btn sr-btn-ghost", onclick: () => cleanup() },
        ["Close"]
      ),
    ]),
  ]);
  ...
  const meta = createEl("div", { class: "sr-meta" }, [
    "Select a snippet to preview its README.",
  ]);
  ...
  const runBtn = createEl(
    "button",
    { class: "sr-btn sr-btn-primary", disabled: "true" },
    ["Run"]
  );
  const copyBtn = createEl("button", { class: "sr-btn", disabled: "true" }, [
    "Copy code",
  ]);
  const openBtn = createEl("button", { class: "sr-btn", disabled: "true" }, [
    "Open in GitHub",
  ]);
...
.sr-backdrop { position: absolute; inset: 0; background: rgba(0,0,0,0.15); }
...
`,
  ...
  function renderList(items) {
    ...
    if (!items.length) {
      list.appendChild(
        createEl("div", { style: { padding: "12px", opacity: 0.8 } }, [
          "No snippets found.",
        ])
      );
      return;
    ...
    for (const s of items) {
      const badges = createEl(
        "div",
        { class: "sr-badges" },
        [
          s.version
            ? createEl("span", { class: "sr-badge" }, [`v${s.version}`])
            : null,
          s.risk
            ? createEl("span", { class: "sr-badge" }, [`risk: ${s.risk}`])
            : null,
          s.tags?.length
            ? createEl("span", { class: "sr-badge" }, [s.tags.join(", ")])
            : null,
        ].filter(Boolean)
      );
  ...
  async function selectSnippet(s, itemEl) {
    // update selection UI
    for (const el of list.querySelectorAll(".sr-item"))
    ...
    try {
      ...
      setMeta(
        `❌ Failed to load selected snippet files. ${String(
          err?.message || err
        )}`
      );
  ...
  async function runSnippet(s, code) {
    try {
      ...
      setMeta(
        `✅ Completed: ${s.name || s.id}${
          result !== undefined ? ` (returned: ${String(result)})` : ""
        }`
      );
      console.info("[SnippetRunner] result:", result);
    } catch (err) {
      setMeta(
        `❌ Error running ${s.name || s.id}: ${String(err?.message || err)}`
      );
  ...
  // initial meta
  setMeta(
    `Loaded ${snippets.length} snippets from manifest. Select one to preview.`
  );
    ...
    snippets: [
      {
        ...
        description:
          "Refreshes the timeline control when it fails to update automatically.",
        version: "1.2.0",
        risk: "low",
        owner: "team-d365",
        tags: ["case", "form", "ui"],
